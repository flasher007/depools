...
impl OrcaWhirlpoolParser {
    pub fn parse_pool_data(&self, data: &[u8]) -> Result<(TokenInfo, TokenInfo, PoolReserves, PoolFees)> {
        ...
        if data.len() < 300 {
            warn!("Data too short for Orca Whirlpool pool");
            return Err(anyhow::anyhow!("Data too short for Orca Whirlpool pool"));
        }

        // Known token positions from our analysis
        let base_mint_start = 101;  // WSOL
        let quote_mint_start = 181; // USDC
        let base_vault_start = 186; // First vault after USDC
        let quote_vault_start = 187; // Second vault
