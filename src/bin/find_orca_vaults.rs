...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔍 Finding Orca Whirlpool vault addresses...");
    println!("Pool: {}", orca_pool);
    ...
    if let Ok(account) = client.get_account(&Pubkey::from_str(orca_pool)?) {
        ...
        // Search for "Orca" string
        if let Some(pos) = find_string(&data, "Orca") {
            println!("  Found 'Orca' at position {}", pos);
        }
        
        // Search for "Whirlpool" string
        if let Some(pos) = find_string(&data, "Whirlpool") {
            println!("  Found 'Whirlpool' at position {}", pos);
        }
        ...
        for i in 0..data.len() - 31 {
            if let Ok(pubkey) = Pubkey::try_from(&data[i..i+32]) {
                ...
                if pubkey_str.contains("whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc") || // Whirlpool
            ...
            if pos + 32 <= data.len() {
                if let Ok(pubkey) = Pubkey::try_from(&data[pos..pos+32]) {
                    ...
                    if pubkey_str == "So11111111111111111111111111111111111111112" ||
                       ...
                       pubkey_str.contains("11111111111111111111111111111111") {
                        continue;
                    }
